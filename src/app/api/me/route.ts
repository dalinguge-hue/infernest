...
export async function GET() {
  ...
  if (!session)   // Get user's aff_code for referrals
  let affCode = "";
  try {
  ...
  let quota = 0;
  let usedQuota = 0;
  let dailyUsage: { day: string; tokens: number }[] = [];
  ...
  try {
    ...
    // Fetch real usage logs for 30-day chart
    ...
    if (logsData.success && logsData.data) {
      ...
      for (const log of logsData.data) {
        ...
        if (dayMap[key] !== undefined) {
          dayMap[key] += log.quota || log.token_count || 0;
        }
      }
      dailyUsage = Object.entries(dayMap).map(([day, tokens]) => ({ day, tokens }));
    }
  } catch (e) {
    console.error("Failed to get user info:", e);
  }

    // Get user's aff_code for referrals
  let affCode = "";
  try {
    const info = await getUserInfo(session.userId);
    if (info.success && info.data) {
